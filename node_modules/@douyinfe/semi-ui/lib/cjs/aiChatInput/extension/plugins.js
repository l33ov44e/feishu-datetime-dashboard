...
exports.specialPasteLogicForInputSlot = specialPasteLogicForInputSlot;
 ...
 */
function specialPasteLogicForInputSlot(event, $from, tr, dispatch, selection) {
  var _a;
  const parentNode = $from.parent;
  const nodeText = parentNode.textContent;
  const isOnlyZeroWidth = nodeText && nodeText === _constants.strings.ZERO_WIDTH_CHAR;
  const isAllTextSelected = !selection.empty && selection.from === $from.start() && selection.to === $from.end();
  if (isOnlyZeroWidth || isAllTextSelected) {
    const pastedText = ((_a = event.clipboardData) === null || _a === void 0 ? void 0 : _a.getData('text/plain')) || '';
    if (pastedText) {
      const pos = $from.start();
      tr = tr.insertText(pastedText, pos, pos + nodeText.length);
      const endPos = pos + pastedText.length;
      tr = tr.setSelection(_prosemirrorState.TextSelection.create(tr.doc, endPos));
      tr.setMeta(_constants.strings.DELETABLE, true);
      dispatch(tr);
      event.preventDefault();
      return true;
